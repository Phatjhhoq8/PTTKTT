...
void BubleSort(Node* ds,int n) {
	for(int i=0;i<n-1;i++) for(int j=n-1;j>i;j--) if(ds[j].key<ds[j-1].key) Swap(&ds[j],&ds[j-1]);
}
int main() {
	int n=0;
	Node* ds = readData(&n);
	...
	printData(ds,n);
	printf("Mang sau khi sap xep dung Buble Sort: \n");
	BubleSort(ds,n);
	printData(ds,n);
	return 0;
}